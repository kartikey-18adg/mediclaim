...
function safeAuthError(error: unknown) { const message = error instanceof Error ? error.message : ''; if (/invalid|credential|password|email/i.test(message)) return new Error('Invalid email or password.'); if (/confirm/i.test(message)) return new Error('Please confirm your email before signing in.'); if (/rate limit/i.test(message)) return new Error('Too many attempts. Please try again later.'); return new Error('Authentication failed. Please try again.'); }
